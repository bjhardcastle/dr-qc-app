...
def get_metrics(qc_path: str):
    return (
        qc_db_utils.get_db(db_path=NAIVE_DB_PATH)
...
def display_rating(
    ...
    df = qc_db_utils.get_db(db_path=NAIVE_DB_PATH).filter(pl.col("qc_path") == path)
...
def next_path(override_next_path: str | None = None) -> None:
    ...
    qc_item_pane.object = display_item(
        current_qc_path, second_image=second_image
    ).object
